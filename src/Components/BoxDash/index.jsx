...
export default function BoxDash({ title, quantity, loadInfo }) {
  return (
    <article className={`flex flex-col w-[100px] sm:w-[180px] items-center ${loadInfo ? 'justify-center' : ''} gap-y-1 bg-green-200 p-5 rounded-xl shadow-md`}>
      {loadInfo ? (
        <ImSpinner8 size={90} className='animate-spin'/>
      ) : (
        <>
          <h2 className="text-sm sm:text-xl text-green-950 font-medium">{title}</h2>
